#BASIC DATA TYPES

#STRING: IS TEXT IN PYTHON, WRITEN IN BETWEEN QUOTATION MARKS, A SEQUENCE OF CHARACTERS
#STRING METHODS:
#upper ()
#lower()
#len()
#replace()

#NUMBER: INTEGERS(int), FLOATS(floats), COMPLEX, 

#CASTING TYPES

#BOOLEANS (bool): True/False VALUES

#NONETYPE: Nothing, none

my_name = 'Maya'
name_upper = my_name.upper()

print (name_upper)

#STRING ARE IMMUTABLE

#SPECIAL CHARS IN STRINGS
sentence = 'I love Python\n'
print(sentence * 3)

sentence = ' I\'m in love with Python\t'
print(sentence * 3)

sentence_js = sentence.replace('Python', 'JavaScript')
print(sentence_js)

price = '15$'
clean_price = int(price.replace('$',''))
print(type(clean_price))

description = "strings are sequence of characters"
print(description.upper())
print(description.replace('are','is'))

#STRING SLICING
print(description[:7]) 

#Variables
f_name = 'Harry'
l_name = 'Potter'
age = 15
address = 'Privet Drive 4'
is_wizard = True

#HOW TO NAME VARIABLES: BEST PRACTICES
# DONT USE NUMBERS OR SPECIAL CHARS (EX: 14 = 'MY FAVE NUMBER')
# USE: SHORT NAMES, IN PYTHON BETTER TO USE UNDERSCORE AS SPACE

first_name = 'Ron'
print(5 == 3)

#the variable doesn't store the expression, but the output of the expression

calculation= 5+6
print(calculation)

general_var = 'Hello'
general_var = 456
general_var = True
print(general_var)

x = 1
y = 2
print (x,y)

#swap values of x and y 
temp = y
y = x
x = temp
print(x,y)

#useful functions:
#print() - it shows something defined on the brackets on the terminal
#input() - prompt the user for some input

user_name = input('Enter your name: ') #inputed info comes as a str
print(user_name)
age = int(input('Enter your age: '))
print(age+10)

print(f'{user_name} is {age} years old')

#f' strings = the f' stands for format()'

#Incrament a variable
count = 0
user_name = input('Enter your name: ')
count += 1
print(user_name)
print(count)

output = ''

output += 'M'
output += 'A'
output += 'Y'
output += 'A'
print(output)

#EXCERCISE
user_age = int(input('What is your age? '))
result = 100 - user_age
print(f'You will turn 100 years old in {result} years')

